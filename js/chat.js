    ...
    deleteBtn.addEventListener('click', async () => {
        ...
        if (confirm('确定删除此文件夹吗？其中的聊天不会被删除，将归入"全部"列表。')) {
            ...
            db.characters.forEach(c => {
                if (c.folderId === currentFolderActionTarget) delete c.folderId;
            });
            db.groups.forEach(g => {
                if (g.folderId === currentFolderActionTarget) delete g.folderId;
            });
...
function renderChatList() {
    ...
    const allChats = [...db.characters.map(c => ({ ...c,
        type: 'private'
    })), ...db.groups.map(g => ({
        ...g,
        type: 'group'
    ...
    if (charStatusBtn) {
        charStatusBtn.addEventListener('click', () => {
            ...
            if (char.statusPanel.history && char.statusPanel.history.length > 0) {
                ...
                slidesData = [{
                    html: char.statusPanel.currentStatusHtml,
                    timestamp: Date.now()
                }];
            }
...
function handleMessageLongPress(messageWrapper, x, y) {
    ...
    if (!isWithdrawn) {
        if (!isImageRecognitionMsg && !isVoiceMessage && !isStickerMessage && !isPhotoVideoMessage && !isTransferMessage && !isGiftMessage && !isInvisibleMessage) {
            menuItems.push({
                label: '编辑',
                action: () => startMessageEdit(messageId)
            });
        }
        ...
        if (!isInvisibleMessage) {
            menuItems.push({
                label: '引用',
                action: () => startQuoteReply(messageId)
            });
        }
        ...
        if (message.role === 'user') {
            menuItems.push({
                label: '撤回',
                action: () => withdrawMessage(messageId)
            });
        }
    }
    ...
    menuItems.push({
        label: '删除',
        action: () => enterMultiSelectMode(messageId)
    });
...
async function saveMessageEdit() {
    ...
    if (isRawEditMode) {
        ...
        if (chat.history[messageIndex].parts) {
            chat.history[messageIndex].parts = [{
                type: 'text',
                text: chat.history[messageIndex].content
            }];
        }
    } else {
...
function createMessageBubbleElement(message, isContinuous = false) {
    ...
    if (bilingualMatch) {
        ...
        bubbleRow.innerHTML = `<div class="message-info"><img src="${avatarUrl}" class="${avatarClass}"><span class="message-time">${timeString}</span></div>`;
    ...
    bubbleRow.innerHTML = `<div class="message-info">${nicknameHTML}<img src="${avatarUrl}" class="${avatarClass}"><span class="message-time">${timeString}</span></div>`;
    ...
    if (bubbleElement) {
        if (quote) {
            ...
            const sanitizedQuotedText = DOMPurify.sanitize(quote.content, {
                ALLOWED_TAGS: []
            });
                ...
                if (msg.role === 'assistant') {
                    ...
                    if (contentHasMatch || partsHasMatch) {
                        if (statusCount < limit) {
                            ...
                            // 双向同步清空：只要 content 或 parts 其中之一被清空，就强制清空另一个
                            // 这确保了无论正则匹配在哪个字段上生效，整条消息都会被标记为空
                            if (msg.content === '') {
                                msg.parts = [];
                            }
        ...
        if (provider === 'gemini') {
            ...
            if (isBackground) {
                contents.push({
                    role: 'user',
                    parts: [{
                        text: `[系统通知：距离上次互动已有一段时间。请以${chat.realName}的身份主动发起新话题，或自然地延续之前的对话。]`
                    }]
                });
            }
                ...
                if (aiQuoteMatch) {
                    ...
                    if (originalMessage) {
                        const message = {
                            ...
                            parts: [{
                                type: 'text',
                                text: `[${character.realName}的消息：${replyText}]`
                            }],
                            timestamp: Date.now(),
...
9. ✨重要✨ 你可以像真人一样撤回你刚刚发送的消息。当你打错了字、说错了话、感到尴尬、或者只是改变了主意时，都可以这样做。这是一个体现你角色性格的机会。格式为：[${character.realName}撤回了一条消息：{被撤回消息的原文}]。
10. ✨重要✨ 你可以选择单独一条消息引用，当你想要引用某句话做出补充或回应时，格式为：[${character.realName}引用“{某条消息内容}”并回复：{回复内容}]。
...
`;
    ...
    if (groups.length > 0) {
        ...
        if (availableStickers.length > 0) {
            ...
            stickerInstruction = `12. 你拥有发送表情包的能力。这是一个可选功能，你可以根据对话氛围和内容，自行判断是否需要发送表情包来辅助表达，你不必在每次回复中都包含表情包。这是你的表情包库：[${stickerNames}]。请使用格式：[表情包：名称]。例如：[表情包：开心]。禁止编造表情包库里没有的表情包。\n`;
            canUseStickers = true;
        }
    }
...
l) 发送并撤回消息: [${character.realName}撤回了一条消息：{被撤回的消息内容}]。这表示你发送了一条消息但立刻后悔并撤回了。`;
...
function setupGroupChatSystem() {
    ...
    const groupAutoSaveInputs = [
        ...
        'setting-group-max-memory', 'setting-group-custom-bubble-css'
    ];
