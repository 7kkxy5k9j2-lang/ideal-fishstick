...
function openProfileCard(charId) {
    ...
    const memorySize = char.history ? JSON.stringify(char.history).length : 0;
    const memoryKB = (memorySize / 1024).toFixed(1);
    document.getElementById('pc-stat-memory').textContent = `${memoryKB}KB`;
    ...
    if (confirm('确定删除当前人设档案吗？')) {
        ...
        setTimeout(() => {
            ...
            if (items[currentPersonaIndex]) {
                items[currentPersonaIndex].scrollIntoView({
                    inline: 'center'
                });
            }
        }, 50);
