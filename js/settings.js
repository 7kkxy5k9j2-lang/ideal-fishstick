...
function openManagePresetsModal() {
    ...
    presets.forEach((p, idx) => {
        ...
        applyBtn.textContent = '应用';
        applyBtn.onclick = function() {
            applyPresetToCurrentChat(p.name);
            modal.style.display = 'none';
        };
    ...
    try {
        if (currentChatType === 'private') {
            ...
            if (e) {
                ...
                if (typeof loadSettingsToSidebar === 'function') try {
                    loadSettingsToSidebar();
                } catch (e) {}
                if (typeof renderChatList === 'function') try {
                    renderChatList();
                } catch (e) {}
            }
        } else {
...
function openManageMyPersonaModal() {
    ...
    presets.forEach((p, idx) => {
        ...
        applyBtn.textContent = '应用';
        applyBtn.onclick = function() {
            applyMyPersonaPresetToCurrentChat(p.name);
            modal.style.display = 'none';
        };
...
function setupPresetFeatures() {
    ...
    if (personaApplyBtn) personaApplyBtn.addEventListener('click', function() {
        ...
        if (!v) return showToast('请选择要应用的预设');
...
function setupWallpaperApp() {
    const e = document.getElementById('wallpaper-upload'),
        t = document.getElementById('wallpaper-preview');
    if (t) {
...
margin-left: auto !important;
    ...
    container.innerHTML = iconsSectionHTML + widgetSectionHTML + fontsSectionHTML + globalCssSectionHTML;
